...
public class Sampling {
	...
	public double averageValueOfSignal(double[] signal){
		double total = 0;
		for(int i = 0; i < signal.length; i++){
			total += signal[i];
		}
		return total/((double) signal.length);
	}
	
	public double averageLogarithmicValueOfSignal(double[] signal){
		double total = 0;
		for(int i = 0; i < signal.length; i++){
			if(signal[i] > 0)
				total += Math.log(signal[i]);
		}
		return total/((double) signal.length);
	}
