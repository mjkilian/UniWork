...
public class Dogs extends JFrame {
   ...
   //database stuff  
   private static final String connStr = "jdbc:oracle:thin:L3_12_1003819k/1003819@crooked.dcs.gla.ac.uk:1521:L3";
   private static Connection conn;
   private static PreparedStatement dogInfoStmt, siblingStmt, childStmt,
                                    grandchildStmt, parentStmt, breedStmt, 
                                    parentBreedStmt,ownerDogsStmt,attendanceStmt;
   ...
   private Dogs() {
      ...
      String ownerCount = ("SELECT ownerid, COUNT(dogid) AS noOfDogs "+
			  "FROM Dog " +
			  "GROUP BY ownerid ");
      
      String ownerDogsQuery = ("SELECT Dog.name "+
    		  				   "FROM Owner, Dog "+
    		  				   "WHERE Dog.ownerid = Owner.ownerid AND Owner.name = ? ");
      
      String attendanceQuery = ("SELECT showname,opendate "+
    		  					"FROM( "+
    		  					 	"SELECT name, showname,opendate "+
    		  					 	"FROM "+
    		  					 	"(SELECT dogid, name "+
    		  					 	 "FROM Dog) "+
    		  					 	 "NATURAL JOIN "+
    		  					 	 "(SELECT dogid,showname,opendate "+
    		  					 	 " FROM ATTENDANCE)) "+
    		  					 "WHERE name = ? "+
    		  					 "ORDER BY opendate ");
      
      try {
    	  ownerDogsStmt = conn.prepareStatement(ownerDogsQuery);
      }catch(SQLException e){
    	  doError(e,"Owner's dogs query will not compile: " + ownerDogsQuery);
      }
      
      try{
    	  attendanceStmt = conn.prepareStatement(attendanceQuery);
      }catch(SQLException e){
    	  doError(e,"Attendance query will not compile: " + attendanceQuery);
      }
      ...
      //own name
      //Result: name
      String siblingQuery = ("SELECT name "+
			    "FROM Dog " +
      ...
      //grandchild 
      //Result: D2.name
      String grandchildQuery = ("SELECT grandchildname " +
    		  					"FROM ( "+
    		  						" SELECT D1.name AS childname, D1.mothername, D1.fathername, D2.name AS grandchildname "+
    		  						" FROM Dog D1, Dog D2 " +
    		  						" WHERE D1.name = D2.mothername OR D1.name = D2.fathername " +
    		  						" ) " +
    		  					" WHERE mothername = ? OR fathername = ? ");
      try {
   ...
   private void redisplay() {
      ...
      Vector <String> ownersDogs = getOwnersDogs(owner);

      if (kennAddr == null) kennAddr = "Unknown";

      //build string with list of owners dogs
      String ownersDogsString = "";
      for(int i = 0; i < ownersDogs.size()-1;i++){
    	  if(ownersDogs.get(i) == null)
    		  continue; //no ancestor
    	  ownersDogsString += ownersDogs.get(i)+", ";
    	  if((i+1)%6 == 0){
    		  ownersDogsString+= "<br>";
    	  }
      }
      ownersDogsString+= ownersDogs.get(ownersDogs.size()-1) + "<br>";
      
      Vector<String> showAndDate = getRecentDogShow(dogName);
      String showstring;
      if(showAndDate == null){
    	  showstring = dogName + " has not attended any show";
      }else{
    	  showstring = "The last show "+ dogName+ " has attended was "+ showAndDate.get(0) +
    	  " on " + showAndDate.get(1);
      }
      ...
      for(int i = 0; i < ancestors.size()-1;i++){
    	  if(ancestors.get(i) == null)
    		  continue; //no ancestor
    	  ancestorsString += ancestors.get(i)+", ";
    	  if((i+1)%6 == 0){
    		  ancestorsString+= "<br>";
    	  }
      }
      ancestorsString+=ancestors.get(ancestors.size()-1) + "<br>";
      
      //build ancestors string
      String descendentsString = "";
      for(int i = 0; i < des.size()-1;i++){
    	  if(des.get(i) == null)
    		  continue; //no descendents
    	  descendentsString += des.get(i)+", ";
    	  if((i+1)%6 == 0){
    		  descendentsString+= "<br>";
    	  }
      }descendentsString+=des.get(des.size()-1) + "<br>";
   ...
   public static void main(String[] args) {
	...
	if (anc == null) anc = new Vector<String>(); //if null create a new vector
	
	if(anc.isEmpty() && dogname == null)//base case (no ancestors found in previous call)
	    return anc;
	...
	try {
		...
		if(dogname != null){ //first usage. Initial dog specified
			parentStmt.setString(1, dogname);
			parentStmt.setString(2, dogname);
		}else{ //recursive usage
			parentStmt.setString(1,anc.get(0));
			parentStmt.setString(2, anc.get(1));
		}
		results = parentStmt.executeQuery();
		
		
		while(results.next()){ //while rows remain
			ancestors.add(results.getString(1));
			ancestors.add(results.getString(2));
		}
		anc.addAll(getAncestors(null,ancestors)); //append the results of recursive query to anc
	}
	catch(Exception e) {
	...
	ResultSet results = null;
	Vector<String> chil = new Vector<String>();
	try {
		childStmt.setString(1,dogname);
		childStmt.setString(2,dogname);
		results=childStmt.executeQuery();
		while(results.next()){
			chil.add(results.getString(1));
		}
		anc.addAll(chil);
		for(String child: chil){
			getDescendents(child,anc);
		}
	}catch(Exception e) {
            doError(e, "Failed to execute ancestor query in getBreeding");
	}
    ...
    private Vector<String> getOwnersDogs(String ownername){
    	...
    	ResultSet results = null;
    	try{
    		ownerDogsStmt.setString(1, ownername);
    		results = ownerDogsStmt.executeQuery();
    		while(results.next()){
    			dogs.add(results.getString(1));
    		}
    	}catch(SQLException e){
    		doError(e,"Failed to execute get owner's dogs query in getOwnersDogs");
    	}
    	return dogs;
    }
    
    private boolean isDateMoreRecentThan(String date1, String date2){
    ...
    private Vector<String> getRecentDogShow(String dogname){
    	Vector<String> showAndDate = new Vector<String>();
    	ResultSet results = null;
    	String showname = null;
    	String showdate = null;
    	try{
    		attendanceStmt.setString(1,dogname);
    		results = attendanceStmt.executeQuery();
    	}catch(SQLException e){
    		doError(e,"Failed to execute get dog's attendance query in getRecentDogShow");
    	}
    	try{
    		boolean first = true;
    		while(results.next()){
    			if(first){
    				showname = results.getString(1);
    				showdate = results.getString(2);
    				first = false;
    			}else{
    				if(isDateMoreRecentThan(showdate,results.getString(2))){
    					showname = results.getString(1);
    					showdate = results.getString(2);
    				}
    			}
    		}
    		showAndDate.add(showname);
    		showAndDate.add(showdate);
    	}catch(SQLException e){
    		doError(e,"Failed to find most recent show in getRecentDogShow");
    	}
    	if(showname == null && showdate == null)
    		return null;
    	else return showAndDate;
